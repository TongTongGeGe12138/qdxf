<template>
  <div class="dwg-to-pdf-bg">
      <div class="dwg-to-pdf-top">
          <h1 class="main-title" style="font-size: 78px;">CAD 转 图片 在线转换</h1>
          <p class="main-desc"
              style="font-family: 'Source Sans Pro', 'sans-serif'; font-size: 24px;color: #7F7F7F;text-align: center;">
              支持 CAD 转 图片，100%免费在线操作，文件自动加密保护，无需安装客户端。一键上传，极速转换，保留矢量信息</p>
                      ...
                      <div v-else>
                          <div class="file-info-box">
                              <div class="icon-close-box"  @click.stop>
                                  <img :src="status === 'success' ? (currentOutput === 'svg' ? svgIcon : pngIcon) : dwgIcon"
                                      :alt="status === 'success' ? (currentOutput === 'svg' ? 'SVG图标' : 'PNG图标') : 'DWG图标'" class="convert-page-icon" />
          ...
          <div class="steps-title">如何免费将 CAD 转换为 图片</div>
          <div class="steps">
              <div class="step">
              ...
              <div class="step">
                  ...
                  <div class="step-text">开始转换<br /><span>点击转换按钮，系统自动处理，无复杂参数设置</span></div>
              </div>
              <div class="step">
                  <div class="step-num">3</div>
                  <div class="step-text">下载图片<br /><span>转换成功点击下载 图片 文件，支持浏览器预览</span></div>
              </div>
          </div>
      </div>
      <div class="dwg-to-pdf-page">
          <div class="section-title main-title">免费在线 CAD 转 图片 工具：极速转换，无需安装</div>
          <div class="desc-center">
              支持各类 DWG 图纸转换为 图片 文件 → 专注单文件快速转换，文件自动加密，转换后立即删除。
          </div>
          <div class="section">
              <div class="section-title section-title-core">核心优势</div>
              <ul class="feature-list">
                  <li>✅ 纯在线操作：无需下载软件或插件，浏览器打开即用</li>
                  <li>✅ 极速转换：10MB文件10秒内完成，保留矢量信息（大型图纸建议切割成单页图纸后上传）</li>
                  <li>✅ 保留矢量线条与基础结构，传统扫描件暂不支持。</li>
          ...
          <div class="section">
              ...
              <ul class="feature-list">
                  ...
                  <li><b>矢量保真</b><br>保留矢量路径，适合网页嵌入与二次编辑</li>
          ...
          <div class="section">
              ...
              <ul class="faq-list">
                  <li><b>转换后路径太复杂？</b><br>复杂图形会生成较多路径，建议按需分层导出。</li>
...
const currentOutput = ref<'svg' | 'png' | null>(null)
...
// PNG 上传别名，便于模板绑定
function handleUploadPng(file: File) {
  return handleUpload(file)
}

// 登录判断封装到 handleUploadSvg
async function handleUploadSvg(file: File) {
  const isValid = await validateToken()
  const currentPath = router.currentRoute.value.fullPath
  if (!isValid) {
      if (typeof localStorage !== 'undefined') {
...
const pngIcon = new URL('../../assets/PNG.png', import.meta.url).href
const svgIcon = new URL('../../assets/SVG.png', import.meta.url).href
...
/* 防止上层样式对所有 img 施加滤镜导致变色 */
.icon-close-box img,
.file-info-box img.convert-page-icon {
  filter: none !important;
  -webkit-filter: none !important;
  mix-blend-mode: normal !important;
}
